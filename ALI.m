...
% This function implements the mathematical equations for the Adaptive 
% Levels of Interval (ALI) Method.
...
T=zeros(1,M);T(1)=2*((2.^M)-1); % Create interval vector.
for L=2:M
    T(L)=(T(L-1)-2)/2;
end
...
NN=(2^(L-1))*floor(N/T(1)) + floor(max(0,(mod(N,T(1))+2-(2.^L))/T(L)));
    for k=1:NN
        ...
        blk=alk+T(L)-1; % bkl;
        % Activity image.
        AIL(:,:,L)=AIL(:,:,L)+abs(I(:,:,alk)-I(:,:,blk)); % AI for each L.
    end
AIL(:,:,L)=AIL(:,:,L)/NN;
end
